...
def decrease_region(gridx, gridy, b, xnext, dynlipconst, drcondfulfilled, traindata, lengthscale, sigmaf, lambdavec):
    ...
    decreaseval = numpy.zeros((gridx.shape[0], gridy.shape[0]))
    ...
    def ma_kernel(x1, x2, l):
        """CUDA device function: Returns covariance between points calculated by Matern 3/2 kernel
        Args:
        ...
        arg = math.sqrt(3 * ((x1[0]-x2[0]) ** 2 / l[0] ** 2 + (x1[1] - x2[1]) ** 2 / l[1] ** 2))
        cov = sigmaf**2 * (1 + arg) * math.exp(-arg)
        return cov

    @cuda.jit(device=True)
    def ma_kernel_derive(deltaup, deltalow, xtrain, xcenter, kappa, l, m):
        """CUDA device function: Returns bounds of the derivative of the Matern 3/2 kernel in dimension m
        Args:
        ...
        if norm == 0:   # special case: training point lies on the interval center (avoid division by 0)
            ...
            kappa[0] = -3 * deltaup * math.exp(-math.sqrt(3.0) * distancenorm) / (l[m] ** 2)    # upper bound
            distancenorm = math.sqrt((deltalow ** 2 / l[m] ** 2) + (b ** 2 / l[m - 1] ** 2))
            kappa[1] = -3 * deltalow * math.exp(-math.sqrt(3.0) * distancenorm) / (l[m] ** 2)   # lower bound
...
def estimate_region_of_attraction(gridx, gridy, traindata, lengthscale, sigmaf, lambdavec, decreaseregion):
    ...
    def calc_gp_mean(devgridx, devgridy, devgpmean):
        ...
        if (i < devgpmean.shape[0]) and (j < devgpmean.shape[1]):
            ...
            meanvalue = 0
            for k in range(0, xtrain.shape[0]):                             # Loop over all training data points
                meanvalue += ma_kernel(x, xtrain[k], l) * lambdavector[k]   # vector product
