...
def decrease_region(gridx, gridy, b, dynamics, dynlipconst, drcondfulfilled, traindata, lengthscale, sigmaf, lambdavec):
    ...
    # calculate ltilde of Matern kernel for abbreviation of formulas
    ltilde = numpy.array((lengthscale[0] / math.sqrt(3), lengthscale[1] / math.sqrt(3)), dtype=numpy.float64)
    # Initialize result array
    decreaseval = numpy.empty((gridx.shape[0], gridy.shape[0]))

    # Required / available VRAM size calculations
    constantvram = traindata.nbytes + ltilde.nbytes + lengthscale.nbytes \
                      + lambdavec.nbytes + b.nbytes + dynlipconst.nbytes + sigmaf.nbytes
    availablevram = cuda.current_context().get_memory_info()[0] - constantvram
    requiredvram = gridx.nbytes + gridy.nbytes + dynamics.nbytes + drcondfulfilled.nbytes + decreaseval.nbytes
    ...
    def ma_kernel(x1, x2, l):
        """CUDA device function: Returns covariance between points calculated by Matern 5/2 kernel
        Args:
        ...
        Returns:
            cov (float64): Covariance between x1 and x2 calculated by the Matern 5/2 kernel"""
        arg = math.sqrt(5.0) * math.sqrt((x1[0]-x2[0]) ** 2 / l[0] ** 2 + (x1[1] - x2[1]) ** 2 / l[1] ** 2)
        cov = sigmaf**2 * (1 + arg + (arg**2)/3) * math.exp(-arg)
        return cov

    @cuda.jit(device=True)
    def ma_kernel_derive(deltaup, deltalow, xtrain, xcenter, kappa, l, m):
        """CUDA device function: Returns bounds of the derivative of the Matern 5/2 kernel in dimension m
        Args:
            deltaup (float64): Distance between xtrain and a point inside the interval with center xcenter
                maximizing the kernel derivative
            deltalow (float64): Distance between xtrain and a point inside the interval with center xcenter
                minimizing the kernel derivative
            ...
            l (tuple): Kernel parameter: separate length scales of ARD kernel used for GP training.
            m (int): Single dimension in which the bounds for the kernel derivative are calculated
        Returns:
            kappa (cuda.local.array): Bounds of the derivative of the Matern 3/2 kernel in dimension m.
        ...
        if norm == 0:   # special case: training point lies on the interval center (avoid division by 0)
            distancenorm = math.sqrt(deltaup ** 2 / l[m] ** 2)
            kappa[0] = -(5 * deltaup / (3 * l[m] ** 2) + math.sqrt(125.0) * deltaup * distancenorm / (3 * l[m] ** 3)) * math.exp(
                -math.sqrt(5.0) * distancenorm)
            distancenorm = math.sqrt((deltalow ** 2 / l[m] ** 2) + (b ** 2 / l[m-1] ** 2))
            kappa[1] = -(5 * deltalow / (3 * l[m] ** 2) + math.sqrt(125.0) * deltalow * distancenorm / (3 * l[m] ** 3)) * math.exp(
                -math.sqrt(5.0) * distancenorm)
            ...
            distancenorm = math.sqrt((deltaup ** 2 / l[m] ** 2) + ((xtrain - xcenter - tauover) ** 2 / l[m-1] ** 2))
            kappa[0] = -(5 * deltaup / (3 * l[m] ** 2) + math.sqrt(125.0) * deltaup * distancenorm / (3 * l[m] ** 3)) * math.exp(
                -math.sqrt(5.0) * distancenorm)
            distancenorm = math.sqrt((deltalow ** 2 / l[m] ** 2) + ((xtrain - xcenter - tauunder) ** 2 / l[m-1] ** 2))
            kappa[1] = -(5 * deltalow / (3 * l[m] ** 2) + math.sqrt(125.0) * deltalow * distancenorm / (3 * l[m] ** 3)) * math.exp(
                -math.sqrt(5.0) * distancenorm)
        return kappa
    ...
    def calc_decrease_region(devgridx, devgridy, devresultgrid, decreaseregion, devxnext):
        ...
        lt = cuda.const.array_like(ltilde)
        ...
        if (i < devresultgrid.shape[0]) and (j < devresultgrid.shape[1]):
            ...
            if decreaseregion[i, j] == 0:
                ...
                for k in range(0, xtrain.shape[0]):     # loop over all training data points
                    ...
                    meanv += ma_kernel(x, xtrain[k], l) * lambdavector[k]
                    # Calculate GP mean at devxnext point
                    meanvf += ma_kernel(devxnext[i][j], xtrain[k], l) * lambdavector[k]

                    for m in range(0, xtrain.shape[1]):
                        # Allocate loc device array to store calculation of upper and lower bounds of kernel derivative
                        kappa = cuda.local.array(shape=2, dtype=numba.float64)

                        # Calculate upper and lower bounds of the GP mean derivative according to the theory:
                        delta = xtrain[k][m] - x[m]
                        if abs(delta) > b + lt[m]:
                            kappa = ma_kernel_derive(abs(delta) - b, abs(delta) + b, xtrain[k][m - 1], x[m - 1], kappa, l, m)
                            if lambdavector[k] > 0:
                                if delta > 0:
                                    upperbound[m] += -kappa[0] * (sigmaf**2) * lambdavector[k]
                                    lowerbound[m] += -kappa[1] * (sigmaf**2) * lambdavector[k]
                                else:
                                    upperbound[m] += kappa[1] * (sigmaf**2) * lambdavector[k]
                                    lowerbound[m] += kappa[0] * (sigmaf**2) * lambdavector[k]
                            else:
                                if delta > 0:
                                    lowerbound[m] += -kappa[0] * (sigmaf**2) * lambdavector[k]
                                    upperbound[m] += -kappa[1] * (sigmaf**2) * lambdavector[k]
                            ...
                            if lambdavector[k] > 0:
                                ...
                                if delta > 0:
                                    ...
                                    lowerbound[m] += kappa[1] * (sigmaf**2) * lambdavector[k]
                                    upperbound[m] += kappa[0] * (sigmaf**2) * lambdavector[k]
                        elif lt[m] - b <= abs(delta) <= lt[m] + b:
                            kappa = ma_kernel_derive(lt[m], abs(delta) + b, xtrain[k][m - 1], x[m - 1], kappa, l, m)
                            # Allocate loc device array to store intermediate result for minimization over +/- b
                            kappam = cuda.local.array(shape=2, dtype=numba.float64)
                            kappam = ma_kernel_derive(lt[m], abs(delta) - b, xtrain[k][m - 1], x[m - 1], kappam, l, m)
                            if lambdavector[k] > 0:
                                if delta > 0:
                                    upperbound[m] += -kappa[0] * (sigmaf**2) * lambdavector[k]
                                    lowerbound[m] += -min(kappa[1], kappam[1]) * (sigmaf**2) * lambdavector[k]
                                else:
                                    upperbound[m] += min(kappa[1], kappam[1]) * (sigmaf**2) * lambdavector[k]
                                    lowerbound[m] += kappa[0] * (sigmaf**2) * lambdavector[k]
                            else:
                                if delta > 0:
                                    lowerbound[m] += -kappa[0] * (sigmaf**2) * lambdavector[k]
    ...
    for indx in range(1, divisionfactor+1):         # on the first axis
        ...
        for indy in range(1, divisionfactor+1):             # on the second axis
            ...
            dynfp1 = cuda.to_device(numpy.ascontiguousarray(dynamics[xstart:xlast, ystart:ylast]), stream=stream)
...
def estimate_region_of_attraction(gridx, gridy, traindata, lengthscale, sigmaf, lambdavec, decreaseregion):
    ...
    def calc_gp_mean(devgridx, devgridy, devgpmean):
        ...
        if (i < devgpmean.shape[0]) and (j < devgpmean.shape[1]):
            ...
            for k in range(0, xtrain.shape[0]):
                meanvalue += ma_kernel(x, xtrain[k], l) * lambdavector[k]
    ...
    for indx in range(1, divisionfactor+1):         # on the first axis
        ...
        for indy in range(1, divisionfactor+1):             # on the second axis
            ...
            # Copy inputs from RAM to GPU memory (H2D)
            start = time.perf_counter()
            dgx = cuda.to_device(numpy.ascontiguousarray(gridx[xstart:xlast]), stream=stream)
            dgy = cuda.to_device(numpy.ascontiguousarray(gridy[ystart:ylast]), stream=stream)
            ...
            # execute GPU calculation
            start = time.perf_counter()
            calc_gp_mean[bpg, tpb, stream](dgx, dgy, dgpmean)
            exectime = time.perf_counter() - start
